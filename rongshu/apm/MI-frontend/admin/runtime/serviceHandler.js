...
class ServiceDirector {
  ...
  beforeExecute (request, response) {
    ...
    if (that.options.validate) {
      let method = that.options.method
      let dataObj = method === 'get' ? request.query : request.body
      that.options.validate.forEach((validater, index) => {
    ...
    if (!validResult.valid) {
      ...
      var beforeRequest = that.options.beforeRequest
      beforeRequest && _.isFunction(beforeRequest) && beforeRequest.call(that, request, response)
    }
  ...
  afterExecute (request, response, actionResult) {
    ...
    afterRequest && _.isFunction(afterRequest) && afterRequest.call(this, request, response, this.actionResult)
  }
  /**
   * Mock数据
