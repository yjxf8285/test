  ...
  for (let key in models) {
    if (!/^__/g.test(key)) {
      apis[key] = function(config = {}, triggerAjaxStartAndStopEvent = true, triggerGlobalEvents = true) {
        ...
        let tierid = util.getLocationParams().tierId || ''
        let instanceid = util.getLocationParams().instanceToken || ''
        // merge data
        ...
        if (allData.condition) {
          if (!allData.condition.tierId) {
            allData.condition.tierId = tierid
          }
          if (!allData.condition.instanceId) {
            allData.condition.instanceId = instanceid
          }
        } else {
          if (!allData.tierId) {
            allData.tierId = tierid
          }
          if (!allData.instanceId) {
            allData.instanceId = instanceid
          }
        }
        ...
        let allConfig = Object.assign({},
          ...
          config, {
            ...
            success(data, statusText, xhr) {
              let debug = false
              if (window.$$apm.env === '<$=env$>') {
                debug = true
              } else {
                debug = window.$$apm.env === 'development'
              }
              if ((data.code && data.code !== 0) && debug) {
                console.error('请求失败的接口名称:', getUrl(key, models[key], registerConfig, config))
                console.error(data)
                if (apiErrorTimer) {
                  clearTimeout(apiErrorTimer)
                  apiErrorTimer = null
                }
                apiErrorTimer = setTimeout(function() {
                  Notification({
                    title: '错误',
                    message: '接口请求失败',
                    type: 'error'
