...
let defaultOptions = {
  ...
  rules: {
    ...
    /**
     * min: [100,'int','输入值不可小于100']
     **/
    min: {
      validator: function(value, param) {
        if (!Array.isArray(param)) {
          param = [param, 'int', '输入值必须大于等于{0}']
        }
        param[2] = param[2] || '输入值必须大于等于{0}'
    ...
    max: {
      validator: function(value, param) {
        if (!Array.isArray(param)) {
          param = [param, 'int', '输入值必须小于等于{0}']
        }
        param[2] = param[2] || '输入值必须小于等于{0}'
        var numType = param[1] = param[1] ? param[1] : 'int'
        if (numType === 'int') {
          return parseInt(value) <= param[0]
        } else if (numType === 'float') {
...
class Validator {
  ...
  validateRule(value, validParams) {
    ...
    if (rule && this.data) {
      var args
      if (typeof validParams !== undefined) {
        args = validParams
      } else {
        args = this.options.validParams || eval(filter[2])
      }
