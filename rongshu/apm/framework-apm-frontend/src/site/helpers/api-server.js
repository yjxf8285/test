...
const ApiSever = {}
const def = {
  url: window.$$framework.apiRootPath ? (window.$$framework.apiRootPath + '/api/') : '/api/', // TODO: alert
  method: 'post',
  baseURL: window.$$framework.apiRootPath ? (window.$$framework.apiRootPath + '/api/') : '/api/', // TODO: alert
...
let checkCode = (response, apiName) => {
  if (response && !response.success) {
    console.error(apiName + ' api request fail:', response.msg || response.message || response.status)
  }
  return response
}
