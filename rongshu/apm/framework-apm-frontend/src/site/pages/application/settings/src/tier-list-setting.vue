...
<template>
  <div class="tier-list">
    <div class="content" ref="loadingContainer">
      ...
      <el-table :data="tierList" class="list-table" style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="renameTier(scope)">
              重命名</el-button>
    ...
    <au-datagrid :columns="datagrid.columns" :data-list="datagrid.data">
      <div slot="name" slot-scope="row">
      </div>
    </au-datagrid>
    <au-datagrid :columns="datagrid.columns" :data-list="datagrid.data">
      <tr slot-scope="row">{{row.value.name + row.value.sex}}</tr>
      <tr slot-scope="row">row.value.sex</tr>
    </au-datagrid>
    </div>
</template>
...
import {nameValidator} from '_helpers/validator'
...
export default {
  ...
  methods: {
    ...
    renameTier (scope) {
      ...
      this.$nextTick(() => {
        /* eslint-disabled */
        document.querySelector('.tier-name-modifying')[scope.$index].querySelector('input')[0].focus()
