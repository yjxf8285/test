...
define(function(require, exports, module) {
    ...
    var CardCalendar=Widget.extend({
        ...
        render:function(){
            ...
            this._initFocusDate();
            /**
             * 轮询的时候检测当天的时间，重置日历当天的日期显示
             * liuxf
             */
            tplEvent.on('globalremind', function (remindData) {
                var dayEl = $('.current-month-date', that.element);
                var serviceTime = moment.unix(remindData.serviceTime).format('YYYY-MM-DD');
                dayEl.each(function () {
                    var dayDate = $(this).attr('data-date');
                    //if (dayDate == '2014-10-24') {
                    if (dayDate == serviceTime) {
                        dayEl.removeClass('is-init-focus-date');
                        $(this).addClass('is-init-focus-date');
                    }
                });
            });
            return result;
