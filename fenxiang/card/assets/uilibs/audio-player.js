...
define(function(require, exports, module) {
    ...
    var audioTpl1=$(tplHelper).filter('.audio-player-tpl-1').html(),
        ...
        audioHtml4=$(tplHelper).filter('.audio-player-tpl-4').html();
    ...
    var AudioPlayer=function(opts){
        opts= _.extend({
            "themeStyle":1, //1、2、3、4四种风格，默认是1风格
            "element":null,
            "audioSrc":"",
            "length":0,  //时长
    ...
    _.extend(AudioPlayer.prototype,{
        "init":function(){
            ...
            if(player){     //存在播放器的条件下
                ...
                }else if(opts.themeStyle==4){
                    elEl.addClass('fs-audio-4');
                    elEl.html(audioHtml4);
                }
                //存储引用
        ...
        _updateStatus1:function(status){
            ...
            }else if(status=="pausing"||status=="stopping"){
        ...
        _updateStatus4:function(status){
            var elEl=this.element,
                iconEl=$('.player-icon',elEl),
                audioLenEl=$('.audio-length',elEl);
            var opts=this.opts;
            if(status=="playing"){
                this._status="playing";
                iconEl.removeClass('mc-play').addClass('mc-pause');
            }else if(status=="pausing"||status=="stopping"){
                this._status="pausing";
                iconEl.removeClass('mc-pause').addClass('mc-play');
            }
            audioLenEl.text(opts.length);   //设置时长
        },
        /**
         * 获取格式化的时长
         * @param len
        ...
        "destroy":function(){
            var elEl=this.element;
            var core=player.core;
            elEl.removeClass('fs-audio fs-audio-1 fs-audio-2 fs-audio-3 fs-audio-4').empty();
            if(player.audioType=="h5"){
