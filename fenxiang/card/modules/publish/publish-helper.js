...
//        publishStyle = require('modules/publish/publish.css');
    ...
    DateSelect.prototype.init = function () {
        ...
        var resetStr='<a class="reset-l" href="javascript:;">清空</a>';
        if(opts.noResteBtn){
            resetStr='';
        }
        var template = '<div class="{{classPrefix}}">' +
                '<div class="fs-publish-dateselect-title">请选择日期'+resetStr+'</div>' +
                '<div class="fs-publish-dateselect-calendar"></div>';
        if(!opts.isCRM) {
            template += '<ul class="{{classPrefix}}-content" data-role="content">' +
                '{{#each select}}' +
        ...
        inputEl.bind('mousedown', function(e){
        	e.stopPropagation();
        });
     ...
     * @returns {*}
     */
    DateSelect.prototype.getTime = function () {
        var opts=this.opts,
            formatStr=opts.formatStr;
        var elEl = this.element,
            inputEl = $('.fs-publish-dateselect-input', elEl),
            dateText=_.str.trim(inputEl.val()),
            dateValue=moment(dateText, formatStr);
        return dateValue ? dateValue._d.getTime()/1000 : 0;
    };
    ...
    TimeSelect.prototype.init = function () {
        ...
        } else if(this.opts.timeDivision == 15){
        	for (i = 0; i < 96; i++) {
                timeModel.push({
                    value: tempTime.format("HH:mm"),
                    text: tempTime.format("HH:mm")
                });
                tempTime = beginTime.add('minutes', 15);
            }
        }
    ...
    var AdjustTextAreaSize=function(opts){
        var elEl;
        opts = _.extend({
            "element": null
        }, opts || {});
    ...
    _.extend(AdjustTextAreaSize.prototype, {
        "destroy": function () {
