...
define(function (require, exports, module) {
    ...
    var FeedList = function (opts) {
        ...
        this.list.GetBatchFilesSource = opts.GetBatchFilesSource;//把附件权限标识传给v
        this.list.crmParam = opts.crmParam; //把crm参数传给v
    ...
    _.extend(FeedList.prototype, {
        ...
        "init": function () {
            ...
            list.on('add', function (m, c, listOpts) {
                ...
                itemV = new ItemV({
                    "model": m,
                    soure: that.list.GetBatchFilesSource,// 1、soure:信息源；2、回复；3、短消息 4、销售合同；5、销售机会；6、产品；7、竞争对手；8、市场活动；9、销售线索； 10、客户；
                    fbrType: that.list.fbrType, // 1，客户关联 2,联系人 3,机会 4，产品 5,对手 6，合同 7，市场活动 8，销售线索
                ...
                /*if(m.cid===c.at(c.length-1).cid){
                 $(root).scrollTop($(root).scrollTop()+30);  //防颤抖
                 }*/
            ...
            this.lazyload = new FsLazyload({
                ...
                "executeHandler": function (activeIndex, cb) {
                    ...
                    if(that.opts.searchSelect) {
                		data = that.opts.searchSelect.getRequestData();
                		data.employeeIDs = data.employeeIDs.join(',');
                		data.keyword = $(that.opts.searchOpts.inputSelector).val();
            ...
            tplEvent.on('globalremind', function (remindData) {
                ...
                if (remindData.success) {
                    if (elEl.is(':visible')) {    //只有在element在显示状态下才出提示
                        ...
                        //if(that.lastMaxId>0&&util.getTplStore("stream","maxFeedId")<that.lastMaxId){   //防止黄条提醒加载完是旧数据
                        //feedRemindEl.hide();
                        //}
                        //更新item view的创建时间
        ...
        fetch: function (requestData, cb) {
            ...
            this.list.fbrType = requestData.fbrType || 0;//把fbrType传给list-v
