...
define(function(require, exports, module) {
    ...
    var DiskFnNewDialog = Dialog.extend({//新建或修改目录功能
        ...
        "show": function() {
            ...
            if (trData) {
                ...
                if (trData.isReadOnly) {
                    radioEl.prop("checked", false).eq(1).prop("checked", true).trigger('click'); //选中只读目录 触发默认事件
                } else {
                    radioEl.prop("checked", false).eq(0).prop("checked", true).trigger('click'); //选中开放目录 触发默认事件
                }
        ...
        "_clear": function() {
            ...
            $('.is-read-only-radio', elEl).prop("checked", false).eq(0).prop("checked", true); //开放目录默认选择
           	infoEl.text('说明：可见范围内的同事都可以上传和删除文件');                         //开放目录默认说明 回归正常文字
    ...
    var DiskFnSendcoDialog = Dialog.extend({//发同事功能
        ...
        "_renderReceipt": function() {
            ...
            var elEl = this.element,
                    isReceiptEl = $('.is-receipt', elEl);
            var receipt = new Receipt({
                "inputSelector": $('.sendco-input', elEl),
                "rangeSb": this.rangeSelectSb,
                "submitCb": function(sbData) {
                    that._setReceiptRange(sbData);
                    this.hide();
        ...
        "show": function() {
            ...
            fileInfoContEl.html('<img src="' + FS.BLANK_IMG + '" alt="" class="fs-attach-' + util.getFileType({
                "name": attachName
            },true) + '-small-icon attachName-img">' + attachName + ' (' + attSize + ')');
            return result;
        },
        "hide": function() {
    ...
    var DiskFnSendclientDialog = Dialog.extend({//发客户功能
        ...
        "show": function() {
            ...
            fileInfoContEl.html('<img src="' + FS.BLANK_IMG + '" alt="" class="fs-attach-' + util.getFileType({
                "name": attachName
            },true) + '-small-icon attachName-img"  class="info-cont-img">' + attachName + ' (' + attSize + ')');
            return result;
        },
        "hide": function() {
    ...
    _.extend(WebDisk.prototype, {
            ...
            var oTable = datatablesEl.dataTable({
                ...
                "aoColumns": [
                    ...
                    {//文件名
                        ...
                        "mRender": function(data, type, full) {
                            ...
                            var fname = '<a href="javascript:void(0);" class="attachName-a"><img src="' + FS.BLANK_IMG + '" alt="" class="fs-attach-' + util.getFileType({
                                "name": name
                            },true) + '-small-icon attachName-img"><span class="attachName-text">' + name + '</span></a>';
                            if (data) {
                                if (full.isReadOnly) {
                    ...
                    {//创建人
                        ...
                        "mData": "creator",
                        "mRender": function(data, type, full) {
                            return '<span class="webdisk-createPerson">' + data.name + '<span class="flag-entity-type">' + (full.isDir ? 1 : 0) + '</span><span class="field-value">' + data.spell + '</span></span>';
                        }
                    },
                    {//创建时间
                        "sWidth": "150px",
                    ...
                    {//操作
                        ...
                        "mRender": function(data, type, full) {
                            ...
                            operatingWarp='<div class="operatingwarp">'+operating+'</div> '
                            return operatingWarp;
                        }
                    }
                ],
                "sAjaxSource": diskUrl,
                    ...
                    oSettings.jqXHR = util.api({
                        ...
                        "success": function(responseData) {
                            ...
                            if (responseData.success) {
                                ...
                                //调整提示文字
                                var emptyTipEl=$('.empty-tip',datatablesEl);    //空tip
                                if(requestData.keyword&&requestData.keyword.length>0){
                                    emptyTipEl.text('没有搜索到文件信息');
                                }else{
                                    emptyTipEl.text('当前目录下没有文件夹和文件');
                                }
                                //重新调整尺寸和位置
                                $(root).resize();
                            }
        ...
        _searchBarInit: function() {
            ...
            searchBtnEl.click(function(evt) {
                ...
                var kwd = _.str.trim(searchInputEl.val());
                that.load({
                    "directoryID": thisDirectory.nDDirectoryID,
                    "keyword": kwd.length>100 ? kwd.substring(0, 100) : kwd,
