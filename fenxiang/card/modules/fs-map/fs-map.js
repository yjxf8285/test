...
//        moduleStyle=require('./fs-map.css'),
    ...
    _.extend(FsMap.prototype,{
        ...
        "_getMapIframe":function(){
            ...
            var elEl=this.element,
                ifrEl=$('.map-ifr',elEl);
            if(ifrEl.length==0){
                ifrEl=$('<iframe class="map-ifr" src="'+FS.MODULE_PATH+'/fs-map/fs-map-container.html" frameborder="0" scrolling="no" />');
                ifrEl.load(function(){
    ...
    var FsMapOverlay=Overlay.extend({
        ...
        events: {
            ...
            'click .map-wrapper':function(evt){
                evt.stopPropagation();  //阻止冒泡
            },
            'click':'hide'  //点击黑色背景，隐藏
        },
        show: function() {
            if (isIE6) {
