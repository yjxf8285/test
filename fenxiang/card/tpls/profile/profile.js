...
define(function(require, exports, module) {
    ...
    _.extend(Profile.prototype, {
        ...
        centerTopInit:function(){
            ...
            //考虑到自己的页面地址栏是没有参数的，所以提前把自己的ID先加上
            if(util.isAsterisk(currentUserData.id,'p')){
                $('.star-big', centerTopEl).attr('data-employeeid',currentUserData.id).addClass('j-starred').addClass('star-big-active').attr('title','取消同事星标');
            }else{
                $('.star-big', centerTopEl).attr('data-employeeid',currentUserData.id).addClass('j-starred').removeClass('star-big-active').attr('title','添加同事星标');
            }

            if(employeeID==0||loginUserData.id==employeeID){
                $('.star-big', centerTopEl).remove();
                ...
                if(employeeData){   //保证在职的人才可以发企信
                    if(employeeID!=0){
                        chatLinkEl.attr('data-employeeid', employeeID).addClass('js-qx-quick-reply').show();
                        if(util.isAsterisk(employeeID,'p')){
                            $('.star-big', centerTopEl).attr('data-employeeid',employeeID).addClass('j-starred').addClass('star-big-active');
                        }else{
                            $('.star-big', centerTopEl).attr('data-employeeid', employeeID).addClass('j-starred').removeClass('star-big-active');
                        }
                    }
                }else{  //离职隐藏
                    chatLinkEl.hide();
        ...
        profileTabsInit:function(){
            var that=this;
            this.searchSelects = [];
    		that.searchSelects[0]=new FsSearchSelect({
    			"associatedIpt": $($('.search-inp', this.tplEl)[0]),
                "trigger": $($('.search-select-btn', this.tplEl)[0]),
                "selectColleague": false,
                "customClass": 'fs-main-feed-list'
            });
    		that.searchSelects[1]=new FsSearchSelect({
    			"associatedIpt": $($('.search-inp', this.tplEl)[1]),
                "trigger": $($('.search-select-btn', this.tplEl)[1]),
                "selectColleague": false,
                "customClass": 'fs-main-feed-list'
            });
            var tplEl=this.tplEl,
        ...
        "feedInit":function(){
            ...
            var feedList = new FeedList({
                ...
                "searchSelect": that.searchSelects[0],
                "searchOpts":{
        ...
        "atfeedInit":function(){
            ...
            var tplEl=this.tplEl,
                tplName=this.tplName,
                profileTabsEl=$('.profile-tabs',tplEl),
                panelEl=$('.atfeed-panel',profileTabsEl),
                filterEl=$('.filter',panelEl),
                feedTypeWEl=$('.feed-type',panelEl),
                subTypeWEl=$('.sub-type',panelEl),
                attachWEl=$('.attach',panelEl),
            ...
            feedList.load();
            filterEl.on('click','.atfeed .filter-value',function(evt){
                var meEl=$(this),
                    subTypeName=meEl.attr('subtypename'),
                    subTypeEl=$('.'+subTypeName+'-type',subTypeWEl);
                $('.filter-field',subTypeWEl).hide();
         ...
         * at我的回复列表
         */
        "atreplyInit":function(){
            var that = this,
            	tplEl=this.tplEl,
                profileTabsEl=$('.profile-tabs',tplEl),
                panelEl=$('.atreply-panel',profileTabsEl),
                filterEl=$('.filter',panelEl),
                feedTypeWEl=$('.feed-type',panelEl),
                attachWEl=$('.attach',panelEl),
                searchEl = $('.search-inp', panelEl),
        ...
        "replyInit":function(){
            ...
            var replyList=new ReplyList({
                ...
                "searchSelect": that.searchSelects[1],
				"searchOpts":{
					"inputSelector":searchEl,
					"btnSelector":searchBtnEl
				},
        ...
        renderEmployeeInfo:function(employeeInfo){
			...
			var pnnHtmlTitle=$('.tpl-c .profile-name-note',tplEl).text();
            $('.tpl-c .profile-name-note',tplEl).attr('title',pnnHtmlTitle);
        },
...
//                                        "planTotalCount":planTotalCount,
            ...
            util.api({
                ...
                "success":function(responseData){
                    ...
                    if(responseData.success){
						...
						//头像信息 移到中间 2014-01-25 by haibo
						$('.myuser-head-img',tplEl).html('<img class="fn-left" src="'+util.getAvatarLink(employeeData.profileImage,1)+'" />');
						
						//头像信息 移到中间 2014-01-25 by haibo end
                        ...
                        if(smsNotDisturb.length>0){
							...
							$('.c-nodisturb',tplEl).html('免打扰 <strong class="color-blue">'+smsNotDisturb[0]+':00~'+smsNotDisturb[1]+':00</strong>');
                        }else{
                            //nodisturbEl.html('未设置免打扰');
							$('.c-nodisturb',tplEl).html('未设置免打扰');
                        }
                        //短信提醒状态
                        if(feedPlanRemaidObject&&feedPlanRemaidObject.weekShow){
							...
							$('.c-sms-alert',tplEl).html('日志提醒 <strong class="color-blue">'+feedPlanRemaidObject.weekShow+'</strong>&nbsp;&nbsp;<strong class="color-blue">'+feedPlanRemaidObject.monthShow+'</strong>');
                        }else{
                            //smsAlertEl.html('还没有设置短信提醒');
							$('.c-sms-alert',tplEl).html('还没有设置消息提醒');
                        }
                        //工作状态提示
                        // $('.tpl-c .working-state',tplEl).html(dataRoot.employee.workingState);
                        //员工名称
                        $('.tpl-c .employee-namel',tplEl).html(dataRoot.employee.name);
                    }
                }
            });
            //自适应左边栏位置
            util.autoAdjustPos($('.tpl-inner',tplLeftEl));
        },
    ...
    exports.init=function(){
        ...
        // 如果当前人是登录用户隐藏@信息
        var isMeHideEl=$('.ui-switchable-nav .if-me-hide',tplEl);
        var contactData = util.getContactData(),
            loginId = contactData["u"].employeeID; //登录ID
        var queryParams = util.getTplQueryParams2();   //取地址栏的参数格式示例 /=/tagType-4
        if(!queryParams||(loginId==queryParams)){
            isMeHideEl.hide();
        }else{
            isMeHideEl.show();
        }
