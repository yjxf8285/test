    ...
    exports.init = function() {
        ...
        $.extend(Feedplanmanages.prototype, {
            ...
            inVipServiceFn: function() {
                ...
                var readmeListEl = $('.readme-list', elEl);
                if (!inVipService) {
                    ...
                    util.api({
                        ...
                        "success": function(responseData) {
                            if (responseData.success) {
                                ...
                                //成功之后
                                var value = responseData.value.value; //可以下载的次数
                                var value1 = responseData.value.value1; //已经下载的次数
                                var remain = value - value1; //剩余下载次数
//                                remain=1;
                                totalDownloadTimeBtn.text(value);

                                if (remain > 0) {
                                    if (parseFloat(remain)/value <= 0.1) { //仅剩10的情况下
