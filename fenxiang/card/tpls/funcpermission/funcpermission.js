...
define(function(require, exports, module) {
    ...
    exports.init = function() {
        ...
        //报数管理员
        var flagReportAdmin = $('#flag-report-admin', tplEl),
                freportListWrap = $('.flag-name-list', flagReportAdmin),
                freportCount = $('.fca-count', flagReportAdmin);
        //促销宝
        ...
        $.extend(ModifyPermissionsPeople.prototype, {
                    ...
                    util.api({
                        ...
                        "success": function(responseData) {
                            if (responseData.success) {
                                ...
                                // list 报数管理员
                                var permissionsDataReport = responseData.value.permissions[41] || [];
                                _.each(permissionsDataReport, function(value) {
                                    value.id = value.employeeID;
                                });
                                freportCount.html(permissionsDataReport.length);
                                if (permissionsDataReport.length > 0) {
                                    var permissionsListHtmlReport = '';
                                    _.each(permissionsDataReport, function(data) {
                                        var name = data.name;
                                        permissionsListHtmlReport += '<li>' + name + '</li>';
                                    });
                                    freportListWrap.html(permissionsListHtmlReport);
                                } else {
                                    freportListWrap.html('无数据！');
                                }
                                // list 促销宝
                                ...
                                that.permissionsDataReport = permissionsDataReport; // list 客户管理员
            ...
            setDialogType: function(elEl) {
                ...
                if (elEl.is('.j-location')) {
                    ...
                    employeesDialog.setTitle('修改签到管理员权限人');
                }
                ...
                if (elEl.is('.j-report')) {
                    this.setInitData(41, this.permissionsDataReport); //报数管理员
                    employeesDialog.setTitle('修改报数管理员权限人');
                }
                if (elEl.is('.j-market')) {
                    ...
                    "success": function(responseData) {
                        if (responseData.success) {
                            ...
                            if (permissionsDataLocation.length > 0) {
                                ...
                                flaListWrap.html('<span class="funcpermission-list-empty-tip">未分配签到管理员。</span>');
                            }
                            // list 客户管理员
                            var permissionsDataCustomer = responseData.value.permissions[31] || [];
                            _.each(permissionsDataCustomer, function(value) {
                            ...
                            if (permissionsDataReport.length > 0) {
                                ...
                                freportListWrap.html('<span class="funcpermission-list-empty-tip">未分配客户管理员。</span>');
                            }
                            // list 促销宝
                            var permissionsDataMarketing = responseData.value.permissions[21] || [];
                            _.each(permissionsDataMarketing, function(value) {
                            ...
                            that.permissionsDataLocation = permissionsDataLocation; // list 定位管理员
                            that.permissionsDataCustomer = permissionsDataCustomer; // list 客户管理员
                            that.permissionsDataReport = permissionsDataReport; // list 报数管理员
                            that.permissionsDataMarketing = permissionsDataMarketing; // list 促销宝
                        }
                    }
                });
