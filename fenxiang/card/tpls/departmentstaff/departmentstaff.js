...
//            var requestData;
    ...
    exports.init=function(){
        ...
        var employeeEditIsValid=function(){
            ...
            var nameReg=/^[-a-zA-Z0-9_\u4e00-\u9fa5]{1,50}$/;
            ...
            if(!nameReg.test(requestData["fullName"])){
                util.alert("姓名中不能包含特殊符号或空格");
                return false;
            }
            ...
            *1开头余下十位皆为数字
            */
            if(requestData["mobile"].length!=11|| !(/^1[0-9]{10}$/.test(requestData["mobile"])) ){
                util.alert('请输入11位大陆手机号');
                return false;
            }
            if(requestData["account"].length==0||!(/^[a-zA-Z\d][\w\.@]{1,49}$/.test(requestData["account"]))){
                util.alert('请输入正确的帐号信息');
                return false;
            }
        ...
        // 应邀人列表的分页
        var invitePagination=new Pagination({
            "element": tplEl.find('.invitepagination-warp'),
            "pageSize":20
        }).render();
        invitePagination.on('page',function(pageNumber){
            var requestData=getEmpListRq()['type'];
            var status=0;
            (requestData==4)?status=1:status=-1;
            getInvitePersonItems({
                status:status,
                pageNumber: pageNumber// int，最后一条的id(0的时候获取最新的数据)
            });
        });

        // 获取应邀人列表
        var getInvitePersonItems = function (oData, type) {
            actionEl.find('.action-btn').addClass('button-state-disabled');//先把功能按钮置灰
            var status;
            var data= _.extend({

                terminal: 3,// int，终端， 0:全部, 1:IOS, 2:Android, 3:PC
                pageSize: 20,// int，分页大小
                pageNumber: 1// int，最后一条的id(0的时候获取最新的数据)
            },oData);
            var elEl = employeeList.element,
                listInnerEl = tplEl.find('.employee-list-box'),
               searchbtnEl = tplEl.find('.icon-search'),
               searchEl = tplEl.find('.search-field'),
                tbodyEl = $('tbody', elEl);
            var htmlStr = '';
            if (type == 4) {
                listInnerEl.removeClass('type-inviteyes').addClass('type-inviteno');
                searchEl.addClass('invite');
                searchbtnEl.addClass('invite');
            } else if (type == 5) {
                listInnerEl.removeClass('type-inviteno').addClass('type-inviteyes');
                searchEl.addClass('invite');
                searchbtnEl.addClass('invite');
            }
            util.api({
                "url": '/Invite/GetInvitePersonItems',
                "type": 'get',
                "dataType": 'json',
                "data": data,
                beforeSend: function () {
                ...
                "success": function (responseData) {
                    if (responseData.success) {
                        ...
                        var personItems=responseData.value.personItems||[];
                        var totalCount=responseData.value.totalCount||0;
                        employeeNumEl.text(totalCount);
                        _.each(personItems, function (item) {
                            var status=item.status||0;
                            var statusStr='';
                            (status==1) && (statusStr='');
                            (status==2) && (statusStr='<span class="fs-green">已通过</span>');
                            (status==3) && (statusStr='<span class="fs-red">已拒绝</span>');
                            htmlStr += '<tr class="edit-disabled">' +
                                '<td class="ck-box first-td"><input class="ck" type="checkbox" /></td>' +
                                '<td><a class="" href="javascript:;">'+item.name+'</a></td>' +
                                '<td><a class="" href="javascript:;">'+item.post+'</a></td><td><a class="" href="javascript:;">'+item.mobile+'</a></td><td><a class="" href="javascript:;">'+statusStr+'</a></td>' +
                                '</tr>';
                        });
                        if (htmlStr.length == 0) {
                            htmlStr += '<tr class="empty-tip"><td>该筛选条件没有找到员工，请更换筛选条件。</td></tr>';
                        ...
                        tbodyEl.html(htmlStr);
                        invitePagination.setTotalSize(totalCount);
                        //保存上一次的请求数据
                        employeeList._lastRequestData=responseData;
                        //保存上一次的返回数据 为了后面选人的时候对比数据
                        employeeList._lastListData=responseData.value.personItems;
                    }
                }
            });
        };
        ...
        filterListEl.on('click','.filter-item',function(evt){
            ...
            var requestData;
            var searchEl = tplEl.find('.search-field'),
            searchbtnEl = tplEl.find('.icon-search');
           var listInnerEl = tplEl.find('.employee-list-box');
            listInnerEl.removeClass('type-inviteyes').removeClass('type-inviteno');
            searchEl.removeClass('invite');
            searchbtnEl.removeClass('invite');
            $('.state-active',filterListEl).removeClass('state-active');
            itemEl.addClass('state-active');
            //分页重置
            paginationAgent('reset');
            invitePagination.reset();// 邀请列表分页重置
            requestData=getEmpListRq();
            // 待审核的邀请
            if(requestData["type"]=="4"){
                getInvitePersonItems({
                    status: 1,//int，状态，0：全部，1：待审核，2：已同意，3：已拒绝, -1: 已审核
                    keyword: $('.search-field',tplEl).val()||''
                },4);
                evt.preventDefault();
                return;
            }
        ...
        //筛选已邀请人列表
        $('.status-list',tplEl).on('change',function(){
            getInvitePersonItems({
                status: $(this).val()//int，状态，0：全部，1：待审核，2：已同意，3：已拒绝, -1: 已审核
            },5);
        });
        //全选和反全选
...
//            employeeList.load(getEmpListRq());
        ...
        actionEl.on('click','.action-btn',function(evt){
            ...
            var employeeIDs= _.map(selectDatas,function(itemData){
                return itemData.employeeID||itemData.invitePersonID;//加入了邀请人的ID
            });
            var accounts;
            var isPassed=true;
            ...
            }else if(btnEl.hasClass('agree-btn')){  //通过
                util.confirm('您确认要用过'+employeeIDs.length+'个用户吗？', '提示', function (evt) {
                    util.api({
                        type: 'post',
                        data: {
                            "invitePersonIDs":employeeIDs.join(',')
                        },
                        url: '/Invite/Agree',
                        success: function (responseData) {
                            if(responseData.success){
            ...
            }else if(btnEl.hasClass('disagree-btn')){  //拒绝
                util.confirm('您确认要拒绝'+employeeIDs.length+'个用户吗？', '提示', function (evt) {
                    util.api({
                        type: 'post',
                        data: {
                            "invitePersonIDs":employeeIDs.join(',')
        ...
        searchBoxEl.on('click','.icon-search',function(){
          ...
          if($(this).is('.invite')){
              ...
              paginationAgent('reset');  //分页重置
              employeeList.load(getEmpListRq()); //重新加载列表
          }
...
//            employeeList.load(getEmpListRq());
        ...
        exportBtnEl.click(function(evt){
            ...
            if(listData&&listData.length>0){
            	var param = getEmpListRq() || {};
            	param._t = +new Date();
                window.open(FS.API_PATH+'/Management/GetAllEmployeesExcel?'+ $.param(param));
            }else{
