...
define(function(require, exports, module) {
    ...
    var MonthWorkDialog=Dialog.extend({
            ...
            util.api({
                ...
                "success":function(responseData){
                    ...
                    if (responseData.success) {
                        ...
                        if (remainTime > 0) {
                            if (parseFloat(remainTime/dataRoot.value) <= 0.1) { //仅剩10%的情况下
                                subBtnEl.text('生成报表（余' + remainTime + '次）');
                                $('.readme-list', elEl).show();
                            }else{
                                subBtnEl.text('生成报表');
                                $('.readme-list', elEl).hide();
                            }
                            subBtnEl.data('remainTime', remainTime);
                            subBtnEl.data('totalTime', dataRoot.value);
                            subBtnEl.removeClass('button-state-disabled');

                        } else {
                            subBtnEl.text('生成报表（余0次）');
                            $('.readme-list', elEl).show();
                        }
        ...
        "_hideDownload":function(evt){
            ...
            if(!this.inVipService()){
                ...
                totalTime=subBtnEl.data('totalTime');
                remainTime--;
                if (parseFloat(remainTime)/totalTime <= 0.1) { //仅剩10%的情况下
                    subBtnEl.text('生成报表（余' + remainTime + '次）');
                    $('.readme-list', elEl).show();
                }else{
                    $('.readme-list', elEl).hide();
    ...
    var CompDialog=Dialog.extend({
        ...
        "events":{
            "click .f-sub":"_submit",
            "click .f-cancel":"_cancel",
            "click .send-date-shortcut .date-shortcut":"_clickSendDateSc",
            "click .approve-date-shortcut .date-shortcut":"_clickApproveDateSc",
            "click .send-date-shortcut .clear-l":"_clearSendDate",
            "click .approve-date-shortcut .clear-l":"_clearApproveDate",
            "click .download-l":"_hideDownload"
        },
        "hide":function(){
        ...
        "show":function(opts){
            ...
            opts && (this.opts = opts);
            if(opts.title) {
            	$('.ui-dialog-title', this.element).html(opts.title);
            }
            if(opts.type==0) {
            	$('.approve-type-field', this.element).show();
            	$('.send-date-field', this.element).hide();
            	$('.include-reply', this.element).show();
            } else {
            	$('.approve-type-field', this.element).hide();
            	$('.send-date-field', this.element).show();
            	$('.include-reply', this.element).hide();
            }
            $('.approve-type', this.element).eq(opts.type||0).attr('checked', true);
            return result;
        },
        "render":function(){
        ...
        "_renderCpt":function(){
            var elEl=this.element,
                ...
                edEl=$('.end-date',elEl),
                ssdEl=$('.send-start-date',elEl),
                sedEl=$('.send-end-date',elEl);
            var contactData=util.getContactData();
            var rangeSb=new SelectBar({
                "element": rangeEl,
                "data": [{
        ...
        "_clickSendDateSc":function(evt){
            ...
            if(meEl.hasClass('current-week')){ //本周
                ssd.setValue(weekRef.clone().startOf('week').add('d', 1));
                sed.setValue(weekRef.clone().endOf('week').add('d', 1));
            }
            if(meEl.hasClass('last-week')){ //上周
                ssd.setValue(weekRef.clone().startOf('week').subtract('w', 1).add('d', 1));
                sed.setValue(weekRef.clone().startOf('week'));
            }
            if(meEl.hasClass('current-month')){ //本月
                ssd.setValue(moment().startOf('month'));
                sed.setValue(moment().endOf('month'));
            }
            if(meEl.hasClass('last-month')){ //上月
                ssd.setValue(moment().startOf('month').subtract('M', 1));
                sed.setValue(moment().startOf('month').subtract('d', 1));
            }
            evt.preventDefault();
        },
        "_clickApproveDateSc":function(evt){
            var meEl=$(evt.currentTarget);
            var sd=this.sd,
                ed=this.ed;
            var weekRef=moment();
            ...
            if(meEl.hasClass('last-week')){ //上周
                ...
                ed.setValue(weekRef.clone().startOf('week'));
            }
            if(meEl.hasClass('current-month')){ //本月
        ...
        "_clearApproveDate":function(evt){
            var sd=this.sd,
                ed=this.ed;
            sd.clear();
            ed.clear();
        ...
        "getRequestData":function(){
            ...
            var sbData=this.sb.getSelectedData(),
                ...
                ssdDate=ssd.getValue(true),
                sedDate=sed.getValue(true);
            var requestData = {};
            if(this.opts && this.opts.type != 0) {
            	requestData={
            		"sendApprovalBeginTime": ssdDate?ssdDate.unix():0,
                    "sendApprovalEndTime":sedDate?sedDate.unix():0,
                    "replyApprovalBeginTime": sdDate?sdDate.unix():0,
                    "replyApprovalEndTime":edDate?edDate.unix():0,
                    "circleIDs": (sbData['g']||[]).join(','),
                    "employeeIDs":(sbData['p']||[]).join(','),
        ...
        "_submit":function(){
            ...
            var coreExecuter=function(){
                util.api({
                    "data":requestData,
                    "url":(that.opts&&that.opts.url) || "/FeedApprove/ExportApprovalSummary",
                    "type":"get",
                    'timeout': 120000,//导出设置超时2min
                    "success":function(responseData){
                        if(responseData.success){
                        	var name = "审批";
                        	if(that.opts && that.opts.type) {
                            ...
                            downloadName=moment.unix(requestData.replyApprovalBeginTime).format('YYYYMMDD')+'-'+moment.unix(requestData.replyApprovalEndTime).format('YYYYMMDD')+ name + '汇总统计.'+
                                util.getFileExtText(responseData.value);
