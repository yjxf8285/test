...
define(function (require, exports, module) {
    ...
    var util = require('util'),
        ...
        FsSearchSelect = require('modules/fs-search-select/fs-search-select'),
        announcement=require('modules/fs-announcement/fs-announcement'),
        FSRemind = require('modules/fs-createremind/fs-createremind'),
        worktodo=require('modules/fs-worktodo/fs-worktodo'),
        FsSelectCity = require('modules/fs-select-city/fs-select-city');
    var fsRemind = new FSRemind(),
    	fsCity = new FsSelectCity();
    ...
    var Stream = function (opts) {
        opts=_.extend({
            ...
            "addFeedCb":FS.EMPTY_FN,    //发布feed后的回调
            "feedlistConfig":{}, //feedlist配置
            "reloadAfterTplSwitch":false   //tpl切换后不刷新
        },opts||{});
        this.opts=opts;
    ...
    _.extend(Stream.prototype, {
        ...
        calMedia: function(){
        	...
        	if($.browser.msie && document.documentMode==7) {
        		...
        		this.timer = setTimeout(function(){
        			var $mediaShare = $('.media-share', that.tplEl),
	            		$media = $($('.media', that.tplEl)[that.activeIndex]);
	            		hMS = $mediaShare.height();
	            	$media.css('height', hMS);
	            	var posM = $media.position();
	            	$mediaShare.css({
	            		top: posM.top-36,
	            		left: posM.left+1
	            	});
        		...
        		var $mediaShare = $('.media-share', this.tplEl),
	        		$media = $($('.media', this.tplEl)[this.activeIndex]);
	        		hMS = $mediaShare.height();
	        	$media.css('height', hMS);
	        	var posM = $media.position();
	        	$mediaShare.css({
         ...
         * 绑定输入框输入时计算media位置
         */
        bindIptMedia: function(){
        	var that = this,
        		$txtarea = $('.publish-input'),
        		$mediaShare = $('.media-share', this.tplEl);
        	$txtarea.on('resizecomplete', function(){
        		that.calMedia(that.activeIndex);
        	});
        },
        ...
        publishInit: function () {
            var that = this;
            var publishWEl = $('.publish-wrapper', this.tplEl),
                publishInputEl = $('.at-input', publishWEl);
            var mediaShareEl=$('.media-share',publishWEl),  //共享media实例
                mediaShare;
            var pageConfig=util.getEnterpriseConfig(107,true)||{};
            this.atInputInit();
            // 点击输入框关闭企信 by liuxf
            publishInputEl.click(function(){
                $('.fs-qx .fs-qx-tbar .hide-l').trigger('click');
                $('.fs-chat-panel .fs-qx-chat-tbar .hide-l').trigger('click');
            });
            mediaShare = new MediaMaker({
                "element": mediaShareEl,
                "action": ["h5imgupload","h5attachupload","vote","at","topic","customer","contacts"],
                "actionOpts": {
                    "vote":{
            ...
            mediaShare.on('filechange', function(){
            	that.calMedia();
            });
            //构建发布框tabs
            var publishTabs = new Tabs({
                ...
                "triggerBgLeft":15, //手动设置切换背景的left值
            ...
            publishWEl.on('click','.panel-visible-h',function(){
                ...
                $('.media-share').show();
                that.calMedia();
            }).on('click','.fs-publish-log-form .publish-input-wrapper fieldset',function(){
                $('textarea',this).get(0).focus();
            });
            //延迟渲染publishTabs里的其他组件
            publishTabs.on('switched',function(toIndex, fromIndex){
                var curPanelEl = publishTabs.get('panels').eq(toIndex),
                    fromPanelEl=publishTabs.get('panels').eq(fromIndex==-1 ? 0 : fromIndex),
                    currentInputEl=$('.publish-input',curPanelEl),  //当前input框
                    fromLastInputEl=fromIndex==3 ? $('.publish-input',fromPanelEl).first() : $('.publish-input',fromPanelEl).last(), //上一个panel中的最后一个input
                    formThumbEl=$('.form-thumb',curPanelEl),    //缩略功能区
...
//                    mediaShareEl.appendTo(mediaHostEl);
                ...
                if(fromInputVal.length>0){
                	(toIndex==3?currentInputEl.first():currentInputEl.last()).val(fromInputVal).keyup().change();
                }else{
                	(toIndex==3?currentInputEl.first():currentInputEl.last()).val('').keyup().change();
                }
                util.setCursorPositionEnd(toIndex==3?currentInputEl.first():currentInputEl.last()); //焦点置于最后
                $(root).scrollTop(0);
                //触发，调整输入框大小
                currentInputEl.trigger('autosize.resize');
                (toIndex==3?currentInputEl.first():currentInputEl.last()).get(0).focus();   //聚焦
                //先清理回执和历史日志引导图
        ...
        isExistContactData: function (name) {
            ...
            if(!found){    //没找到员工，开始找部门
                found= _.find(contactGroupData, function (val) {
                    if (val.name == name && val.id != 999999) {
                        return true;
                    }
                });
            }
        ...
        "planInit": function () {
            ...
            var clear = function () {
                ...
                *查看历史日志提示 暂时清除
                */
                $('.fs-guide-history-plan-bracket').remove();
                //直属领导清理
        ...
        "approveInit": function () {
            ...
            var publishWEl = $('.publish-wrapper', this.tplEl),
                ...
                tripEl = $('.trip', approveWEl),
                loanEl = $('.load', approveWEl),
                tripWrapperEl = tripEl.parents('.publish-extend-panel'),
                leaveRowTpl = $('.leave-row-tpl', approveWEl).html(),
                baoxiaoRowTpl = $('.baoxiao-row-tpl', approveWEl).html(),
                tripRowTpl = $('.trip-row-tpl', approveWEl).html(),
            ...
            var addLeaveRow = function (rowTpl) {
                ...
                var timeEditEl = $('.time-span-edit',rowEl);
                var startDate,
                    startTime,
                    endDate,
                    endTime,
                ...
                startDate.on('change', function () {
                    ...
                    timeEditEl.val(util.countTime(startDate,startTime,endDate,endTime));
                    conuttTotalTime();
                });
             ...
             * 动态生成一条差旅单
             */
            var addTripRow = function(rowTpl){
            	if(tripEl.find('.trip-num-edit').length >= 20) return;
            	var rowEl = $(rowTpl || tripRowTpl);
                var startDate,
                    startTime,
                    endDate,
                    endTime,
                ...
                rowEl.appendTo(tripEl);
                tripEl.find('.trip-num-edit').last().val(tripEl.find('.trip-num-edit').length);
                //初始化请假类型
                /*new Select({
                 trigger: $('.cate-wrapper select',rowEl)
                 }).render();*/
                ...
                $('.trip-wrapper').bind('mousedown', function(e){
                	document.body.click();
                });
                //清空input change事件
                $('.departure-edit', tripEl).unbind('selected');
                $('.departure-edit', rowEl).bind('selected', function () {
                    addTripRow();
                });
                return rowEl;
            };
            ...
            var getRequestData = function () {
                ...
                var approveExtTabsAi = approveExtTabs.get('activeIndex'),   //获得审批人当前active index tab
                    approveExtTabPanelEl = approveExtTabs.get('panels').eq(approveExtTabsAi),
                    approveExtSb = $($('.selectbar', approveExtTabPanelEl)[0]).data('sb'),   //当前激活tab下的selectbar组件
                ...
                //差旅单
                if(requestData["approveType"]=="4") {
                	requestData["approveDetail"] = {};
                	var tripersExtSb = $('.trip-tripers', approveExtTabPanelEl).data('sb'),
                		tripersExtSbData = tripersExtSb.getSelectedData();
                	requestData["approveDetail"]["memberIDs"] = tripersExtSbData['p'] || [];
                	//默认将出差人信息放入可视范围
                    if(requestData["approveDetail"]["memberIDs"].length>0){
                        $.merge(requestData["employeeIDs"],requestData["approveDetail"]["memberIDs"]);
                    }
                    requestData["approveDetail"].approveReason = tripWrapperEl.find('.trip-reason').val();
                    requestData["approveDetail"].approveBudget = parseFloat(tripWrapperEl.find('.trip-budget').val()) || 0;
                    requestData["approveDetail"].approvePrepayments = parseFloat(tripWrapperEl.find('.trip-advance').val()) || 0;
                    requestData["approveDetail"]['items'] = [];
                    $('tbody tr',tripEl).each(function(){
                        var trEl=$(this);
                        var beginDate=$('.start-date-wrapper input',trEl).val(),
                            beginTime=$('.start-time-wrapper input',trEl).val(),
                            endDate=$('.end-date-wrapper input',trEl).val(),
                            endTime=$('.end-time-wrapper input',trEl).val();
                        requestData["approveDetail"]['items'].push({
                        	"idx": $('.trip-num-edit', trEl).val(),
                            "startPoint": $('.departure-edit', trEl).text(),
                            "endPoint": $('.destination-edit', trEl).text(),
                            "beginDate":beginDate,  //for 验证
                            "stopDate":endDate,   //for 验证
                            "beginTime":beginTime,  //for 验证
                            "stopTime":endTime,    //for 验证
                            "startTime":beginDate?moment(beginDate+" "+beginTime,'YYYYMMDD HH:mm').unix():0,
                            "endTime":endDate?moment(endDate+" "+endTime,'YYYYMMDD HH:mm').unix():0,
                            "vehicle": parseInt($('.vehicle',trEl).val()),
                            "daysNumber": parseInt(_.str.trim($('.stay-days-edit',trEl).val()))
                        });
                    });
                }
                //借款单
                if(requestData["approveType"]=="5") {
                	requestData["approveDetail"] = {};
                	requestData["approveDetail"]["reason"] = $('.loan-reason', loanEl).val();
                	requestData["approveDetail"]["amount"] = parseFloat(loanEl.find('.loan-amount').val());
                }
                //清除全空行
                if(requestData["approveType"]=="4"){
                	requestData["approveDetail"]["items"]= _.reject(requestData["approveDetail"]["items"],function(itemData){
                        var isEmpty=true;
                        _.find(itemData,function(itemValue, pkey){
                            if(pkey != 'idx') {
                            	if(!!itemValue){
                                    isEmpty=false;
                                    return true;
                ...
                } else if(requestData["approveType"]!=5) {
            ...
            var isValid = function () {
                ...
                if(approveType=="4"){   //差旅单
                	if(approveDetail.memberIDs.length<=0) {
                		util.alert("请选择出差人");
                		passed = false;
                		return false;
                	}
                	if(!$.trim(approveDetail.approveReason)) {
                		util.alert("请输入出差事由");
                		passed = false;
                		return false;
                	}
                	if(approveDetail.approveBudget && !(parseInt(approveDetail.approveBudget)>=0 && parseInt(approveDetail.approveBudget)<100000)) {
                		util.alert("预算金额最小0元，最多不超过10万元");
                		passed = false;
                		return false;
                	}
                	if(approveDetail.approvePrepayments && !(parseInt(approveDetail.approvePrepayments)>=0 && parseInt(approveDetail.approvePrepayments)<100000)) {
                		util.alert("预支金额最小0元，最多不超过10万元");
                		passed = false;
                		return false;
                	}
                    passed=!(_.find(approveDetail.items,function(itemData){
                        if(!itemData["startPoint"]){
                            util.alert("请填写行程" + itemData.idx + '的出发地');
                            return true;
                        }
                        if(itemData["startTime"]==0){
                            util.alert("请填写行程" + itemData.idx + '的出发时间');
                            return true;
                        }
                        if(!itemData["endPoint"]){
                            util.alert("请填写行程" + itemData.idx + '的目的地');
                            return true;
                        }
                        if(itemData["endTime"]==0){
                            util.alert("请填写行程" + itemData.idx + '的到达时间');
                            return true;
                        }
                        if(moment.unix(itemData["startTime"]).isAfter(moment.unix(itemData["endTime"]))){
                            util.alert("结束时间需大于开始时间");
                            return true;
                        }
                        if(!itemData["vehicle"]){
                            util.alert("请选择行程" + itemData.idx + "的交通工具");
                            return true;
                        }
                        if(itemData["daysNumber"] !=0 && (!itemData["daysNumber"] || isNaN(itemData["daysNumber"]))){
                            util.alert("请正确填写行程" + itemData.idx + "的住宿天数，只能为正整数，不住宿填0");
                            return true;
                        }
                        if(!(parseInt(itemData["daysNumber"])>=0 && parseInt(itemData["daysNumber"])<1000)) {
                    		util.alert("住宿天数最少0天，最多不超过1000天");
                    		return true;
                    	}
                    }));
                }
                if(approveType=="5"){   //借款单
                	if(!$.trim(approveDetail.reason)) {
                		util.alert("请输入借款事由");
                		passed = false;
                		return false;
                	}
                	if(!$.trim(approveDetail.amount)) {
                		util.alert("请输入借款金额");
                		passed = false;
                		return false;
                	}
                	if(isNaN(approveDetail.amount)){
                        util.alert("请输入正确的借款金额");
                        passed = false;
                		return false;
                    }
                	if(!(parseInt(approveDetail.amount)>0 && parseInt(approveDetail.amount)<100000)) {
                		util.alert("借款金额最小1元，最多不超过10万元");
                		passed = false;
                		return false;
                	}
                }
             ...
             * 初始化差旅单
             */
            var tripInit = function(){
            	for (i = 0; i < tripMinRowNum; i++) {
                    addTripRow();
                }
            	tripWrapperEl.find('.trip-reason').val('').trigger('autosize.resize');
            	tripWrapperEl.find('.trip-budget').val('');
            	tripWrapperEl.find('.trip-advance').val('');
            };
            /**
             * 初始化借款单
             */
            var loanInit = function(){
            	$('.loan-loaner', loanEl).val(util.getCurrentEmp().name);
            	$('.loan-reason', loanEl).val('').trigger('autosize.resize');
            	$('.loan-amount', loanEl).val('');
            };
            /**
             * 发布后清理
             */
            var clear = function () {
                ...
                $('.trip-tripers', approveWEl).data('sb').removeAllItem();
                $('.trip-tripers', approveWEl).data('sb').addItem({
                	"id":util.getCurrentEmp().id,
                    "name":util.getCurrentEmp().name,
                    "type":"p"
                });
                //发送短信重置
                sendSmsCbEl.prop('checked', false);
            ...
            //差旅单
            tripEl.on('change', 'input.stay-days-edit', function(){
            	var meEl = $(this),
	                v = _.str.trim(meEl.val());
	            if (v.length > 0) {
	                v = parseFloat(v);
	                if (!isNaN(v)) {
	                    v = parseFloat(v.toFixed(2));
	                    meEl.val(v.toFixed(0));
	                } else {
	                    meEl.val('');
	                }
	            }
            });
            tripWrapperEl.find('.trip-reason').on('change', function(){
        		tripWrapperEl.find('.trip-reason').trigger('autosize.resize');
        	});
            tripEl.on('click', '.departure-edit, .destination-edit', function(e){
            	fsCity.show($(this));
        	});
            //差旅单预算、预支
            tripWrapperEl.on('change', 'input.trip-budget, input.trip-advance', function(){
            	var meEl = $(this),
	                v = _.str.trim(meEl.val());
	            if (v.length > 0) {
	                v = parseFloat(v);
            ...
            //借款单借款金额
            loanEl.on('change', 'input.loan-amount', function(){
            	var meEl = $(this),
	                v = _.str.trim(meEl.val());
	            if (v.length > 0) {
	                v = parseFloat(v);
        	...
        	loanEl.find('.loan-reason').on('change', function(){
        		loanEl.find('.loan-reason').trigger('autosize.resize');
        	});
            //设置多媒体功能
            /*media = new MediaMaker({
            ...
            $('.selectbar', approveExtendEl).each(function () {
                var $this = $(this),
                	isTripers = $this.hasClass('trip-tripers');
                if(isTripers) {
                	defaultSelectedData=[{
                        "id":util.getCurrentEmp().id,
                        "name":util.getCurrentEmp().name,
        ...
        feedInit: function () {
            var that = this;
            var opts=this.opts;
            this.searchSelect=new FsSearchSelect({
                "trigger": $('.search-select-btn', this.tplEl),
                "associatedIpt": $('.search-inp', this.tplEl),
                "selectColleague": true,
                "customClass": 'fs-main-feed-list'
            });
            ...
            feedListTabs.on('switched',function (toIndex, fromIndex) {
                ...
                var feedType = curPanelEl.attr('feedtype'),
                    ...
                    feedRemindClickCb,
                    listEmptyText;
                if (feedType == "0" || feedType == "1" || feedType=="9998") {   //全部和分享和销售记录
                    ...
                    } else if(feedType=="9998") {
                    	listEmptyText = "您所属的部门中还没有人发布过CRM信息";
                    }
                    if (!feedList) {
                        feedList = new FeedList(_.extend({
                            ...
                            "listSuccessCb":function(responseData){
                                ...
                                if(feedList.lastMaxId>lastMaxId){
                                    ...
                                    //重启30s轮询
                                    FS.circleRemind('stop');
                                    FS.circleRemind('start');
                                }
                                opts.feedListSuccessCb&&opts.feedListSuccessCb.apply(this,arguments);
                            },
                            "searchSelect": that.searchSelect,
            ...
            tpl.event.on('switched', function (tplName2, tplEl) {
                if (tplName2 == that.tplName) {
                    ...
                    if(!firstRender){
                        ...
                        that.schedule&&that.schedule.refreshScheduleDate(); //刷新日历
                        //开启feedlist lazyload
                        that.currentFeedList && that.currentFeedList.lazyload.open();
                        //直接刷新
                        if(opts.reloadAfterTplSwitch){
                            reloadFirstTab(feedListTabs);  //切换后重刷新列表
                        }
                    }
                    firstRender=false;
                }else{
                    that.currentFeedList && that.currentFeedList.lazyload.pause();
                }
            });
	...
	//console.info(responseData) 邀请同事列表的数据
						...
						if(responseData.success){
							...
							if(inCircleEmployeeIDs.length>16){
								inCircleEmployeeIDs=inCircleEmployeeIDs.slice(0,16);
							}
							//先清空内容
							...
							if(votes.length<1){
								$('.right-votes-title-wrap').parent().hide();
							}else{
								$('.right-votes-title-wrap').parent().show();
							}
							...
							//右侧提醒
							var remindEl = $('.feed-remind-wrapper', tplEl);
							if(remindEl[0]) {
            ...
            (function(){
            	var remindEl = $('.feed-remind-wrapper', tplEl),
            		btnAdd = $('.add-remind-btn', remindEl);
                    ...
                    if(responseData.success){
                        ...
                        if(listData.length<1){
                            ...
                            _.each(listData,function(data,i){
                                var anTime=data.createTime||data.isShowTime, //首先考虑显示公告创建时间，然后是显示时间
                                    anTitle=data.title,
                ...
                "success":function(responseData){
                    if(responseData.success){
                        ...
                        if(listData.length<1){
                            ...
                            _.each(listData,function(data,i){
                                var anTime=data.createTime||data.isShowTime,
                                    anTitle=data.title,
                                    anFeedId=data.feedID;
                                htmlArr[i]='公告：<time class="color-999999">'+moment.unix(anTime).format('MM-DD')+'</time> '+anTitle+'（<a href="#stream/showfeed/=/id-'+anFeedId+'" class="fna-blue">详情</a>）';
                            });
                            ...
                            util.api({
                                ...
                                'success': function (responseData) {
                                    if (responseData.success) {
                                        ...
                                        //隐藏media
                                        $('.media-share', that.tplEl).hide();
                                    }
                                    sendCb();   //media send callback回调
                                },
                                'error': function(){
                                	sendCb();   //timeout时也要去掉遮罩
    ...
    Events.mixTo(Stream);
    //进入快速体验帐号
    var EnterDemoDialog=Dialog.extend({
        "attrs":{
            width: '440px',
            className:'enter-demo-dialog',
            content:'<div class="ui-dialog-title">快速体验</div>'+
                '<div class="ui-dialog-body">'+
//                    '<div class="f-field"><input type="text" class="captcha auto-focus" /><img src="'+FS.BLANK_IMG+'" alt="" class="captcha-img" /></div>'+
                    '<span>是否进入模拟体验帐号？</span>'+
                '</div>'+
                '<div class="ui-dialog-bbar fn-clear">'+
                    '<button type="button" class="f-sub button-submit">进入</button>&nbsp;&nbsp;<button type="button" class="f-cancel button-cancel">取消</button>'+
                '</div>'+
            '</div>'
        },
        "events":{
            "click .f-sub":"_submit",
            "keydown .captcha":"_keydownCaptcha",
            "click .f-cancel":"_cancel",
            "click .captcha-img":"_clickCaptcha"
        },
        "_submit":function(evt){
            var that=this;
            var elEl=this.element,
                captchaEl=$('.captcha',elEl),
                subBtnEl=$('.f-sub',elEl);
            var captchaVal= _.str.trim(captchaEl.val()),
                captchaKey=this.captchaKey;
//            if(captchaVal.length==0){
//                util.showInputError(captchaEl);
//                return false;
//            }
            util.api({
                type: 'get',
...
//                    "source":3
                ...
                url: FS.BASE_PATH+"/DemoAccount/SimpleLogin",
                success: function(responseData) {
                    var items;
                    var gotoUrl;
                    if(responseData.success){
                        gotoUrl=responseData.data;
                        //设置返回正式帐号标志
                        clientStore.set('openReturnAccount', true);
                        location.href=FS.BASE_PATH+"/"+gotoUrl;
                    }else{
                        util.alert('登录失败，'+responseData.error);
                        that.updateCaptchaKey();
                    }
                }
            },{
        ...
        "_clear":function(){
            ...
            captchaEl.val("");
            captchaImgEl.attr('src',FS.BLANK_IMG);
        },
        "updateCaptchaKey":function(){
            var now=new Date();
            var elEl=this.element,
                captchaImgEl=$('.captcha-img',elEl);
            this.captchaKey=Math.ceil(now.getTime()/1000);
            captchaImgEl.attr('src',FS.BASE_PATH+'/DemoAccount/GetCodeImg?key='+this.captchaKey);
        },
        "show":function(){
            var result=EnterDemoDialog.superclass.show.apply(this,arguments);
            this.updateCaptchaKey();
            return result;
        },
        "hide":function(){
            var result=EnterDemoDialog.superclass.hide.apply(this,arguments);
            this._clear();
            return result;
        },
        "destroy":function(){
            var result;
            result=EnterDemoDialog.superclass.destroy.apply(this,arguments);
            return result;
        }
    });
    exports.init = function () {
        var tplEl = exports.tplEl,
            ...
            stream,
            enterDemo;
        var fHelperEl=$('.fxiaoke-helper',tplEl),
            feedListEl=$('.home-center-list',tplEl);
        var appStore=FS.getAppStore('contactData'),
            assistentConfig=appStore["assistentConfig"],
            htmlStr='';
        //过滤出OpenBrowser类型的外链
        assistentConfig= _.filter(assistentConfig,function(itemConfig){
            return itemConfig.funcCode=="OpenBrowser";
        });
        //排序
        assistentConfig= _.sortBy(assistentConfig,function(itemConfig){
            return itemConfig.order;
        });
        if(util.getContactData()['u'].isAdmin){
            $('.with-admin',fHelperEl).show();
            $('.without-admin',fHelperEl).hide();
        }else{
            $('.with-admin',fHelperEl).hide();
            $('.without-admin',fHelperEl).show();
        }
        //和子页面公共部门处理,左边栏部分
        tplCommon.init(tplEl,tplName);
        //拼接纷享助手
        _.each(assistentConfig,function(itemConfig){
            htmlStr+='<li class="action-item"><a href="'+FS.API_PATH+itemConfig.webUrl+'" target="_blank">'+itemConfig.displayName+'</a>';
        });
        $(htmlStr).appendTo($('.helper-actions .action-list',fHelperEl));
        //纷享助手中的邀请同事链接
        fHelperEl.on('click','.invite-employee-l',function(evt){
            $('.tpl-r .colleague-section .invite-apv',tplEl).click();
            evt.preventDefault();
        });
        //纷享助手中的进入体验帐号
        enterDemo=new EnterDemoDialog();
        fHelperEl.on('click','.enter-demo-l',function(evt){
            enterDemo.show();
            evt.preventDefault();
        });
        //stream初始化
        stream = new Stream({
            "feedListSuccessCb":function(responseData,requestData){
                var feedType=requestData.feedType,
                    ...
                    feedListInnerEl;
                if(responseData.success&&responseData.value.items.length<15&&keyword.length==0){  //15是每次懒加载feedList数量
                    feedListEl=$('.home-center-list',tplEl);
                    ...
                    //显示纷享助手
                    if(!appStore.isDemoAccount){    //不是演示帐号才显示纷享助手
                        //fHelperEl.insertAfter(currentFeedList.element);
                        fHelperEl.appendTo($('.feed-list-inner',currentFeedList.element));
                        fHelperEl.show();
                    }
                    //隐藏feed列表
                    //feedListEl.addClass('fn-hide-abs');
                }else{
                    fHelperEl.hide();
                    //feedListEl.removeClass('fn-hide-abs');
                }
            }
            /*"addFeedCb":function(){
                fHelperEl.hide();
