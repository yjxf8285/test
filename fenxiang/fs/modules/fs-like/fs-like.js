...
define(function(require, exports, module) {
	...
	var LikelistC = Backbone.Collection.extend({
		...
		parse: function(responseData) {
			...
			var newItems, fmfeedType;
			if (responseData.success) {

				newItems = util.deepClone(responseData.value.feedLikes);
				_.each(newItems, function(item) {
					item.sendername = item.employee.name; //发出赞的人
					item.profileImage = util.getAvatarLink(item.employee.profileImage, 2); //发出赞的人头像
					item.employeeID = item.employee.employeeID; //发出赞的人id
