;(function (w, $, r, u) {
    ...
    HistoryList.prototype = {
        ...
        // 渲染模板
        _template: '<li class="{{SenderStatus}}" data-id="{{TemplateID}}">{{TemplateName}}</li>',
        ...
        _bindEvents: function () {
            ...
            me.wrapper.on('click', 'li', function () {
                ...
                referrer = 'list';
                routie('detail/' + $(this).attr('data-id'));
            });
        },
        ...
        render: function() {
            ...
            $.each(listData, function(index, item) {
                item.SenderStatus = item.SenderStatus == 2 ? 'new' : '';
    ...
    /**
     * @description 历史详情 编辑 选择审核人 构造函数
     * @constructor
     *
     */    
    function HistoryDetail(opt){
    ...
    HistoryDetail.prototype = {
        
        constructor: HistoryDetail,
        ...
        _bindEvents: function () {
            ...
            $el.on('click', '.data-wrapper tbody tr', function () { // 查看详情信息
                ...
                that.statusEdite ? routie('edite/item') : routie('edite/show');
            })
            .on('click', '.js_edite', function () { // 编辑事件
                var $this = $(this);
                
                if (!that.statusEdite) {
                    $this.text('确定');
                    $this.parent().addClass('edite-on');
                    that.statusEdite = true;
                    if(that.rowsData.length > 0) {
                        unit.showToast('点击列表进入编辑模式');
                    }
                } else {
                    $this.text('编辑');
                    $this.parent().removeClass('edite-on');
                    that.statusEdite = false;
                    if(that.rowsData.length > 0) {
                        unit.showToast('点击列表进入查看模式');
                    }                    
                }
            })
        ...
        getDetailData: function() {
            ...
            unit.api({
                'type': 'get',
                'url': '/W/Baoshu/GetDataReportHistory',
                'data': {
                    'templateId': that.templateId,
                    'reportDay': that.datepicker.getDate().split('-').join('')
                },
                'success': function(data) {
                    data = data.Value;
        ...
        initDate: function() {
            var that = this;
            if (this.datepicker) { // 访问不同模板时,重置时间为当前时间
                this.datepicker.reset();
                return;
            }
            this.datepicker = new Datepicker({
                target: $('.con', that.wrapper),
                callBack: function() {
                    that.getDetailData();
                }
            });
        ...
        /**
         * @desc渲染非编辑状态下的edite页面
         * 
         */
        renderEditeShow: function() {
            var that = this;
            var index = that.curItemIndex,
                item = that.rowsData[index],
                lis = '<ul>' + 
                        '<li><span>' + item.CommonDataDetail.NodeName + '</span></li>';
            
            $.each(item.DataItems, function(i, mItem) {
                lis += '<li class="show-item">' +
                            '<span>' + mItem.ItemName + '</span>' +
                            '<input disabled="true" value="' + mItem.Value + '" />' +
                       '</li>';
            });
            $('.footer', that.editeWrapper).hide();
        ...
        renderByStatus: function() {
            var $el = $('.footer', this.wrapper),
                $dateBtn = $('.date-bar .con', this.wrapper);
                
            if (this.statusEdite) {
                if (this.statusChange) {
                    $el.addClass('sub-on');
                    $dateBtn.addClass('data-change');
                    this.renderRows();
                } else {
                    $el.addClass('edite-on');
                }
            }
        },
        ...
        refresh: function(id) {
            ...
            $('.date-bar .con', this.wrapper).removeClass('data-change');
            this.getDetailData();
        },
         ...
         */
        init: function(id) {
            if (this.templateId == id && !this.statusSub && referrer != 'list') { // id无变化并没有提交 读取缓存
                this.renderByStatus();
                this.show();
                return;
            }
